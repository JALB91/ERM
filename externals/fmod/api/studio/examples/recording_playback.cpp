...
State executeRecord(FMOD::Studio::System* system)
{
	...
	for (;;)
	{
		...
		Common_Draw("Use the arrow keys (%s, %s, %s, %s) to control the engine position", Common_BtnStr(BTN_LEFT), Common_BtnStr(BTN_RIGHT), Common_BtnStr(BTN_UP), Common_BtnStr(BTN_DOWN));
		Common_Draw("Press %s to quit", Common_BtnStr(BTN_QUIT));
