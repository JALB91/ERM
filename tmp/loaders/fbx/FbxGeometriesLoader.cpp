...
#include "erm/resources/models/IndexData.h"
	...
	if (deformerCount > 0 && bonesTree)
	{
		for (int deformerIndex = 0; deformerIndex < deformerCount; ++deformerIndex)
		{
			...
			for (int clusterIndex = 0; clusterIndex < clusterCount; ++clusterIndex)
			{
				...
				StringID targetNameID (cluster->GetName());
				if (!targetNameID.IsValid())
					targetNameID = linkNode->GetName();
				
				ERM_ASSERT(targetNameID.IsValid());
				...
				bonesTree->ForEachDo([&targetNode, &targetNameID](BonesTree& node) {
					if (!targetNode && node.GetPayload().mName == targetNameID)
						targetNode = &node;
				});
				...
				for (int animIndex = 0; animIndex < scene->GetSrcObjectCount<FbxAnimStack>(); ++animIndex)
				{
					...
					auto it = std::find_if(animations.begin(), animations.end(), [&animationName](SkeletonAnimation& animation) {
//						TODO: Damiano
						(void)animation;
						(void)animationName;
//						return animation.mResourceID == animationName;
						return false;
					});
					...
					{
						const float totalAnimationTime = (static_cast<float>(animationLength) / static_cast<float>(targetFps)) * scale;
						animation = &animations.emplace_back(animationName, {}, totalAnimationTime);
					}
					else
					{
						animation = &(*it);
	...
	mutex.lock();
	auto& meshes = resourcesManager.GetMeshes();
	auto& mesh = meshes.emplace_back(std::make_unique<Mesh>(std::move(vData), std::move(iData), pMesh->GetName()));
	model.AddMesh(*mesh);
	mutex.unlock();
}
