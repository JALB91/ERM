	...
	while (std::getline(stream, line))
	{
		...
		if (splitted.size() > 0)
		{
			if (splitted[0].compare("o") == 0)
			{
				...
				{
					AddMesh(mutex, resourcesManager, model, indicesOffset, std::move(verticesData), std::move(indicesData), material, meshName);
					meshName.clear();
				}
			}
			...
			else if (splitted[0].compare("usemtl") == 0)
			{
				...
				auto it = std::find_if(materials.begin(), materials.end(), [materialName](Material& material) {
//					TODO: Damiano
					(void)material;
//					return material->mName.compare(materialName) == 0;
					return false;
				});
				material = it != materials.end() ? &(*it) : nullptr;
				mutex.unlock();
			}
	...
	std::string& meshName)
{
	...
	mutex.lock();
	auto* mesh = resourcesManager.RegisterResource(Mesh(meshName, std::move(verticesData), std::move(indicesData)));
	ERM_ASSERT_HARD(mesh);
	model.AddMesh(*mesh);
	mutex.unlock();
}
