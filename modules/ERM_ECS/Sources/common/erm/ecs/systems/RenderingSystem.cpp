...
//	const auto* material = erm::ObjectRegistry::require<erm::AssetsRepo>().getAsset<erm::Material>(materialID);
//	const auto* pbMaterial = erm::ObjectRegistry::require<erm::AssetsRepo>().getAsset<erm::PBMaterial>(materialID);
...
//
//	for (auto& data : renderingComponent.mRenderData)
//	{
//		data.mMeshes.clear();
//	}
//	
//	auto& assetsRepo = erm::ObjectRegistry::require<erm::AssetsRepo>();
//
//	for (u64 i = 0; i < meshes.size(); ++i)
//	{
//		auto* mesh = assetsRepo.getAsset<Mesh>(meshes[i]);
//
//		if (mesh == nullptr) {
//			continue;
//		}
...
//		auto* targetShaderProgram = assetsRepo.getAsset<ShaderProgram>().getOrCreateShaderProgram(getShaderForConfig(pipelineConfigs, light, skeletonComponent).c_str());
//
...
, mAssetsManager(ObjectRegistry::require<AssetsManager>())
