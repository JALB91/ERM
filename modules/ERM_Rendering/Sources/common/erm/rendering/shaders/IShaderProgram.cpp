...
std::vector<erm::u8> ReadShaderCompiled(std::string_view path)
{
	...
	erm::u64 fileSize = static_cast<erm::u64>(stream.tellg());
	std::vector<erm::u8> buffer(fileSize);
...
std::vector<erm::u32> LoadSpirvFile(const std::vector<erm::u8>& compiledShader)
{
	std::vector<erm::u32> buffer(compiledShader.size() / sizeof(erm::u32));
...
{
	const auto makeUboData = [&compiler, &resource](erm::UboId id, erm::u64 size) -> erm::UboData {
		return {
...
void IShaderProgram::setShadersSources(const ShaderSources& shadersSources)
{
	for (const auto& [type, sources] : shadersSources)
	{
		...
		{
			fs::writeToFile(ShaderUtils::getShaderFilename(mPath, static_cast<u32>(i), type).c_str(), sources[i]);
		}
...
void IShaderProgram::updateShadersData(ShaderType shaderType)
{
	...
	while (true)
	{
		...
		d.mShaderSource = fs::readFromFile(shaderPath.c_str());
...
void IShaderProgram::updateBindingData()
{
	...
	for (const auto& [shaderType, shadersData] : mShadersDataMap)
	{
		...
		{
			ERM_ASSERT(data.mShaderCompiler != nullptr);
