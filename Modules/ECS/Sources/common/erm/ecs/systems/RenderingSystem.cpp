...
////	const auto* material = erm::gAssetsModule.GetAssetsRepo().GetAsset<erm::Material>(materialID);
...
//	{
//		auto* mesh = gAssetsModule.GetAssetsRepo().GetAsset<Mesh>(meshes[i]);
//
//		if (mesh == nullptr) {
//			continue;
//		}
...
////		auto* targetShaderProgram = gAssetsModule.GetAssetsRepo().GetAsset<ShaderProgram>().GetOrCreateShaderProgram(GetShaderForConfig(pipelineConfigs, light, skeletonComponent).c_str());
////
...
//	const math::mat4 viewInv = glm::inverse(view);
	...
	mModelSystem->ForEachComponent([&](ModelComponent& component) {
		...
		if (!component.GetModelID().IsValid())
			return;
//
//		auto* model = gAssetsModule.GetAssetsRepo().GetAsset<Model>(component.GetModelID());
