...
namespace erm {
	...
	{
		profiling[id] = std::make_pair(Game::GetInstance().GetTimer().GetCurrentTime(), false);
	}
	
	void Profiler::EndProfiling(const char* id)
	{
		profiling[id] = std::make_pair(Game::GetInstance().GetTimer().GetCurrentTime() - profiling[id].first, true);
	}
